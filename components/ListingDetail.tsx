...
export const ListingDetail: React.FC<ListingDetailProps> = ({ listing, onBack, onUpdate, onNext, uiLang }) => {
  ...
  // 初始化安全检查：如果 listing 结构异常，直接报错返回
  if (!listing || !listing.cleaned) {
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center space-y-4">
        <AlertCircle size={48} className="text-red-500" />
        <h3 className="text-xl font-black text-slate-800">Data Error</h3>
        <p className="text-slate-400 max-w-sm">Unable to load listing details. The data might be corrupted or missing.</p>
        <button onClick={onBack} className="px-6 py-2 bg-slate-900 text-white rounded-xl font-bold">Return to List</button>
      </div>
    );
  }
  ...
  // 当外部 listing ID 变化时（例如点击 Next），重置本地状态
  useEffect(() => {
  ...
  const allImages = [
    localListing.cleaned?.main_image || '', 
    ...(localListing.cleaned?.other_images || [])
  ].filter(img => img !== '');
  ...
  const handleLocalFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const updated = { ...localListing };
      if (!updated.cleaned) throw new Error("Missing cleaned data");
        ...
        <ImageEditor 
          ...
          onSave={async (base64) => {
            try {
              ...
              const currentImages = [localListing.cleaned?.main_image || '', ...(localListing.cleaned?.other_images || [])].filter(x => x);
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
            ...
            <div className="relative aspect-square rounded-3xl bg-slate-50 border border-slate-100 overflow-hidden mb-6 group shadow-inner transition-all hover:shadow-xl">
              {selectedImage ? (
                <img src={selectedImage} className="w-full h-full object-contain transition-transform duration-700 group-hover:scale-110" alt="Main" loading="lazy" />
              ) : (
            ...
            <div className="grid grid-cols-4 gap-3">
              {allImages.map((img, i) => (
                ...
                >
                  <img src={img} className="w-full h-full object-cover rounded pointer-events-none" alt={`Thumbnail ${i}`} loading="lazy" />
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <input 
                      type="number"
                      step="0.01"
                      value={localListing.cleaned?.price || 0}
               ...
               <div className="p-8 space-y-8 flex-1 overflow-y-auto custom-scrollbar">
                 <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <CharCounter count={currentContent.optimized_title?.length || 0} limit={LIMITS.TITLE} />
                    </div>
                    <textarea 
                 ...
                 <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <CharCounter count={currentContent.search_keywords?.length || 0} limit={LIMITS.KEYWORDS} />
                    </div>
                    <input 
                      ...
                      onBlur={handleBlur}
                      className={`w-full px-5 py-4 bg-slate-50/50 border ${(currentContent.search_keywords?.length || 0) > LIMITS.KEYWORDS ? 'border-red-500 ring-4 ring-red-100' : 'border-slate-200'} rounded-2xl text-sm font-mono tracking-tight text-slate-600 focus:border-amber-500 transition-all shadow-inner`}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                           ...
                           <div key={i} className="space-y-2 p-4 rounded-2xl border border-slate-50 hover:bg-slate-50/30 transition-all animate-in fade-in slide-in-from-top-2">
                             <div className="flex justify-between items-center">
                               ...
                               <div className="flex items-center gap-3">
                                 <CharCounter count={f?.length || 0} limit={LIMITS.BULLET} />
                                 <button onClick={() => {
                             ...
                             <textarea 
                               ...
                               onBlur={handleBlur}
                               className={`w-full p-4 bg-white border {(f?.length || 0) > LIMITS.BULLET ? 'border-red-500 ring-4 ring-red-100' : 'border-slate-200'} rounded-xl text-xs font-bold text-slate-700 outline-none transition-all focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 min-h-[80px] leading-relaxed shadow-sm`}
                             />
                           </div>
                         ))}
                       </div>
                    ...
                    <div className="space-y-2">
                       ...
                       <textarea 
                         ...
                         onBlur={handleBlur}
                         className={`w-full p-6 bg-white border {(currentContent.optimized_description?.length || 0) > LIMITS.DESCRIPTION ? 'border-red-500 ring-4 ring-red-100' : 'border-slate-200'} rounded-3xl text-xs font-medium text-slate-700 min-h-[500px] leading-loose transition-all focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none shadow-sm`}
                       />
                    </div>
                 </div>
               </div>
