...
import { 
  ...
  Trash2, Layers, Minus, Eraser, Hand
} from 'lucide-react';
...
export const ImageEditor: React.FC<ImageEditorProps> = ({ imageUrl, onClose, onSave, uiLang }) => {
    ...
    const loadImg = async () => {
      ...
      try {
        ...
        img.onload = async () => {
          await img.decode();
          setCanvasImage(img);
          const scale = Math.min((window.innerWidth - 500) / img.width, (window.innerHeight - 300) / img.height, 1);
          setZoom(scale);
  ...
  useEffect(() => { draw(); }, [elements, selectedId, canvasImage, strokeColor, strokeWidth, fontSize]);
  ...
  const draw = () => {
    ...
    elements.forEach(el => {
      ...
      if (el.type === 'brush' && el.points) {
        ctx.beginPath(); if (el.points.length > 0) { ctx.moveTo(el.points[0].x, el.points[0].y); el.points.forEach(p => ctx.lineTo(p.x, p.y)); } ctx.stroke();
      } else if (el.type === 'rect') { ctx.strokeRect(el.x, el.y, el.w || 0, el.h || 0); }
      else if (el.type === 'rect_fill') { ctx.fillRect(el.x, el.y, el.w || 0, el.h || 0); }
      else if (el.type === 'circle') { ctx.beginPath(); const r = Math.sqrt((el.w||0)**2 + (el.h||0)**2); ctx.arc(el.x, el.y, r, 0, Math.PI * 2); ctx.stroke(); }
      else if (el.type === 'line') { ctx.beginPath(); ctx.moveTo(el.x, el.y); ctx.lineTo(el.x + (el.w || 0), el.y + (el.h || 0)); ctx.stroke(); }
      ...
      if (el.id === selectedId) {
        ctx.globalCompositeOperation = 'source-over'; ctx.setLineDash([5, 5]); ctx.strokeStyle = '#6366f1'; ctx.lineWidth = 2 / zoom;
  ...
  const handleStart = (e: React.MouseEvent) => {
    ...
    if (currentTool === 'hand' || currentTool === 'select') {
      setIsPanning(true); setDragStart({ x: e.clientX - offset.x, y: e.clientY - offset.y });
    ...
    const newEl: Element = { id, type: currentTool as any, x: pos.x, y: pos.y, w: 0, h: 0, color: strokeColor, strokeWidth, fontSize, points: ['brush', 'erase'].includes(currentTool) ? [pos] : [] };
    if (currentTool === 'text') {
      const t = prompt("Text:"); if (!t) { setIsDragging(false); return; }
  ...
  const handleStandardize = async () => {
    ...
    setTimeout(() => {
      const buffer = document.createElement('canvas'); buffer.width = 1600; buffer.height = 1600;
  ...
  const handleFinalSave = async () => {
    ...
    setTimeout(() => {
      canvasRef.current!.toBlob(async (blob) => {
        if (!blob) return setIsProcessing(false);
        const fd = new FormData(); fd.append('file', blob, `edited_${Date.now()}.jpg`);
        try {
          const res = await fetch(TARGET_API, { method: 'POST', body: fd });
          const data = await res.json();
          const rawSrc = Array.isArray(data) && data[0]?.src ? data[0].src : data.url;
    ...
    <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col font-inter overflow-hidden">
      <div className="h-16 bg-slate-900 border-b border-white/10 px-6 flex items-center justify-between text-white shrink-0 z-50 shadow-2xl">
        <div className="flex items-center gap-6">
          ...
          <span className="font-black text-xs uppercase tracking-widest text-indigo-400">Media Studio Pro v4.7</span>
        </div>
      ...
      <div className="flex-1 flex relative overflow-hidden">
        ...
        <div className="flex-1 relative bg-slate-950 cursor-grab active:cursor-grabbing overflow-hidden" onWheel={handleWheel} onMouseDown={handleStart} onMouseMove={handleMove} onMouseUp={() => { setIsDragging(false); setIsPanning(false); }} onMouseLeave={() => { setIsDragging(false); setIsPanning(false); }}>
          {isProcessing && <div className="absolute inset-0 z-[60] bg-slate-950/40 backdrop-blur-md flex flex-col items-center justify-center"><Loader2 className="animate-spin text-indigo-500 mb-4" size={48} /><p className="text-[10px] font-black text-white/50 uppercase tracking-[0.3em]">Syncing Pixels...</p></div>}
          <div style={{ transform: `translate(${offset.x}px, ${offset.y}px) scale(${zoom})`, transformOrigin: 'center' }} className="inline-block shadow-[0_0_100px_rgba(0,0,0,0.5)] bg-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><canvas ref={canvasRef} className="block" /></div>
          <div className="absolute bottom-8 right-8 px-5 py-2.5 bg-slate-900/80 backdrop-blur-md border border-white/10 rounded-full text-[10px] font-black text-white/50 shadow-2xl">{Math.round(zoom * 100)}% ZOOM</div>
        </div>
        <div className="w-72 bg-slate-900 border-l border-white/5 p-8 space-y-10 shrink-0 z-50 overflow-y-auto">
           ...
           <div className="space-y-5">
              ...
              <div className="grid grid-cols-5 gap-2">
                 {['#ff0000', '#00ff00', '#0000ff', '#ffff00', '#ff00ff', '#00ffff', '#000000', '#ffffff', '#ff8800', '#6366f1'].map(c => (<button key={c} onClick={() => setStrokeColor(c)} style={{backgroundColor: c}} className={`aspect-square rounded-lg border-2 transition-all ${strokeColor === c ? 'border-white scale-110 shadow-lg' : 'border-transparent opacity-60'}`} />))}
              </div>
           </div>
        </div>
      </div>
