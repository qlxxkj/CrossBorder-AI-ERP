...
export const AuthPage: React.FC<AuthPageProps> = ({ onBack }) => {
  ...
  const handleSocialLogin = async (provider: 'google' | 'wechat') => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: provider as any, // 'wechat' typically requires specific configuration in Supabase
        options: {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <button onClick={onBack} className="absolute top-8 left-8 flex items-center text-slate-500 hover:text-slate-800 transition-colors">
        ...
        <div className="p-8 pb-6">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 text-sm mt-2">
              {isSignUp ? 'Start optimizing your listings today.' : 'Sign in to access your dashboard.'}
            </p>
          </div>
          ...
          <div className="space-y-3 mb-6">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-2.5 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors font-medium text-slate-700 bg-white"
            >
          ...
          {message && (
            <div className={`mb-4 p-3 rounded-lg text-sm flex items-center gap-2 ${message.type === 'error' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
              {message.text}
            </div>
          )}
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-slate-900 text-white rounded-lg font-bold hover:bg-slate-800 transition-colors flex items-center justify-center"
            >
